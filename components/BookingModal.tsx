  ...
  return (
    ...
    >
      <div
        ...
        className="relative w-full max-w-md bg-white dark:bg-[#111c24] rounded-2xl sm:rounded-3xl shadow-2xl border border-slate-200/80 dark:border-slate-800 p-4 sm:p-5 max-h-[90vh] overflow-y-auto"
      >
        
        {/* Close Button Top Right */}
        <button
          onClick={handleModalClose}
          className="absolute top-3.5 right-3.5 p-1.5 rounded-xl text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 transition active:scale-95 cursor-pointer z-10"
        ...
        {isSubmitted ? (
          ...
          <div className="py-2 text-center space-y-3.5 animate-in zoom-in-95 duration-200">
            ...
            <div className="mx-auto w-14 h-14 rounded-full bg-emerald-100 dark:bg-emerald-950/60 border border-emerald-300 dark:border-emerald-700 flex items-center justify-center text-emerald-600 dark:text-emerald-400 shadow-sm">
              <CheckCircle2 className="w-8 h-8" />
            </div>

            <div className="space-y-1">
              <h3 className="text-lg sm:text-xl font-extrabold text-slate-900 dark:text-slate-50 font-heading">
                {t.booking.successTitle}
              </h3>
              <p className="text-[11px] sm:text-xs text-slate-600 dark:text-slate-300 max-w-sm mx-auto leading-relaxed">
                {language === "hi" ? (
                  <>
            ...
            {/* Summary Box */}
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 text-left space-y-2 text-[11px]">
              ...
              <div className="grid grid-cols-2 gap-2 text-slate-600 dark:text-slate-300">
                <div>
                  <span className="text-[9px] text-slate-500 dark:text-slate-400 block">{t.booking.serviceLabel}:</span>
                  <span className="font-semibold text-slate-800 dark:text-slate-200 truncate block">{service}</span>
                </div>
                <div>
                  <span className="text-[9px] text-slate-500 dark:text-slate-400 block">{t.booking.homeSizeLabel}:</span>
                  <span className="font-semibold text-slate-800 dark:text-slate-200">{homeSize}</span>
                </div>
              </div>

              {notes && (
                <div className="pt-1 border-t border-slate-200/60 dark:border-slate-800 text-[10px]">
                  <span className="text-[9px] text-slate-500 dark:text-slate-400 block">{t.booking.notesLabel}:</span>
            ...
            <div className="space-y-2 pt-1">
              <a
                ...
                className="w-full py-2.5 px-4 rounded-xl font-bold text-xs sm:text-sm text-white bg-[#25D366] hover:bg-emerald-600 active:scale-98 transition flex items-center justify-center gap-2 shadow-xs cursor-pointer"
              >
              ...
              <button
                type="button"
                onClick={handleModalClose}
                className="w-full py-2 px-4 rounded-xl font-semibold text-xs text-slate-700 dark:text-slate-200 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 transition active:scale-98 cursor-pointer"
              >
                {t.booking.doneClose}
              </button>
            </div>
          ...
          <form onSubmit={handleSubmitEnquiry} className="space-y-3">
            ...
            <div className="space-y-0.5 pr-6">
              <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-primary dark:text-emerald-400 bg-primary/10 dark:bg-emerald-500/10 px-2 py-0.5 rounded-full">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-1.5 sm:py-2 text-xs font-medium bg-slate-50 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700 rounded-xl text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary focus:outline-none transition shadow-2xs"
              >
                {SERVICES.map((s) => {
            ...
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-1 flex items-center justify-between">
              ...
              <div className="grid grid-cols-5 gap-1.5">
                {["1 RK", "1 BHK", "2 BHK", "3 BHK", "Villa"].map((size) => (
                  <button
                    ...
                    className={`py-1.5 px-1 text-[11px] font-semibold rounded-lg border text-center transition cursor-pointer active:scale-95 ${
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1 sm:gap-1.5">
                {AGRA_LOCALITIES.slice(0, 6).map((loc) => (
                  <button
                    key={loc}
                    type="button"
                    onClick={() => setLocality(loc)}
                    className={`px-2 py-0.5 text-[10px] sm:text-[11px] rounded-md border font-medium transition cursor-pointer active:scale-95 ${
                      locality === loc
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-2.5 py-1.5 text-xs bg-slate-50 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary focus:outline-none transition shadow-2xs"
                  />
                </div>
              </div>
            ...
            {/* 5. Optional Note */}
            <div>
              <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 mb-0.5">
              ...
              <input
                ...
                className="w-full px-2.5 py-1.5 text-xs bg-slate-50 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700 rounded-lg text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary focus:outline-none transition shadow-2xs"
              />
            </div>

            {/* Trust Pill */}
            <div className="p-2 rounded-lg bg-slate-50 dark:bg-slate-900/60 border border-slate-200/60 dark:border-slate-800 flex items-center justify-between text-[10px] text-slate-600 dark:text-slate-400">
              <div className="flex items-center gap-1.5">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                <span>{language === "hi" ? "100% सत्यापित स्टाफ" : "100% Verified Staff • Zero Advance"}</span>
              </div>
              <a
            ...
            <button
              ...
              className={`w-full py-2.5 sm:py-3 px-4 rounded-xl font-bold text-xs sm:text-sm transition-all duration-200 flex items-center justify-center gap-2 ${
                isFormValid
            ...
            >
              <PhoneCall className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isFormValid ? "text-white" : "text-slate-400 dark:text-slate-500"}`} />
